...
public class DeviceListActivity extends Activity {
    ...
    private Set<String> pairedDeviceId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.device_llist);
       ...
       // 添加已配对设备到列表并显示
        ...
        // findViewById(R.id.title_paired_devices).setVisibility(View.VISIBLE);
            for (BluetoothDevice device : pairedDevices) {
                 ...
                 pairedDeviceId.add(device.getAddress());
             }
         } else {
...
//            bundle.putString(EXTRA_DEVICE_ADDRESS, address); //data为你要传递的值
//            msg.setData(bundle);// 传入值
//            mHandler.sendMessage(msg);// mHandler传递参数 ；mHandler为全局变量

            // 设置返回数据
            Intent intent = new Intent();
            intent.putExtra("msg_BtAddress", address);
//            // 设置返回值并结束程序
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                }else if(pairedDeviceId.contains(device.getAddress()) == false){   //添加到已配对设备列表
                    mPairedDevicesArrayAdapter.add(device.getName() + "\n" + device.getAddress());
                }
